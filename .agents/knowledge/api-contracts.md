...
- The version number tracks the behaviour of the CLI and the Action, not the `core` API. `core` is published only because `github-personal-stats` depends on it by version and would not install otherwise; its README and description say so, and direct dependents are told to pin exactly. A `core` API break is therefore a minor release, and the CLI, the Action inputs, the `options` names, and the rendered output are what a major release is reserved for.
- That licence covers the shape of the surface, not its soundness. `HeatRamp` stays opaque and constructed only through `parse` or `Default`, and no public type may describe a state its methods cannot serve: a reachable panic is a defect at any version number.
...
- `heat` and `metric` are card kinds like any other, and `--metric` selects a figure using the same names `--stat-rows` and `--streak-sides` accept. One vocabulary covers panels and tiles, so `stars` cannot mean different things in different places.
- `--height auto`, `--padding`, and `--scale` reach `GithubStatsConfig` through the same builders and `options` passthrough as every other flag. All three default to previous behaviour, so committed output is unchanged. `auto` is refused for the cards that divide a height between sections rather than being silently ignored.
- `scale_basis_points` is an integer for the same reason the shares are: exact comparison and deterministic output. It affects the SVG's `width` and `height` only; the `viewBox` stays in layout units.
