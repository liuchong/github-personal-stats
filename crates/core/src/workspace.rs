use crate::error::GithubStatsError;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OutputKind {
    Dashboard,
    Stats,
    Languages,
    Streak,
    Heat,
    Metric,
    Repo,
    Gist,
    Activity,
    ActivityReadme,
    Status,
    Json,
    Png,
}

impl OutputKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::Dashboard => "dashboard",
            Self::Stats => "stats",
            Self::Languages => "languages",
            Self::Streak => "streak",
            Self::Heat => "heat",
            Self::Metric => "metric",
            Self::Repo => "repo",
            Self::Gist => "gist",
            Self::Activity => "activity",
            Self::ActivityReadme => "activity-readme",
            Self::Status => "status",
            Self::Json => "json",
            Self::Png => "png",
        }
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorkspaceInfo {
    pub name: &'static str,
    pub version: &'static str,
    pub default_output: OutputKind,
    pub supported_outputs: Vec<OutputKind>,
}

impl WorkspaceInfo {
    pub fn to_json(&self) -> String {
        let supported_outputs = self
            .supported_outputs
            .iter()
            .map(|output| format!("\"{}\"", output.as_str()))
            .collect::<Vec<_>>()
            .join(", ");

        format!(
            "{{\n  \"name\": \"{}\",\n  \"version\": \"{}\",\n  \"default_output\": \"{}\",\n  \"supported_outputs\": [{}]\n}}",
            self.name,
            self.version,
            self.default_output.as_str(),
            supported_outputs
        )
    }
}

pub fn workspace_info() -> WorkspaceInfo {
    WorkspaceInfo {
        name: "github-personal-stats",
        version: env!("CARGO_PKG_VERSION"),
        default_output: OutputKind::Dashboard,
        supported_outputs: vec![
            OutputKind::Dashboard,
            OutputKind::Stats,
            OutputKind::Languages,
            OutputKind::Streak,
            OutputKind::Heat,
            OutputKind::Metric,
            OutputKind::Repo,
            OutputKind::Gist,
            OutputKind::Activity,
            OutputKind::ActivityReadme,
            OutputKind::Status,
            OutputKind::Json,
            OutputKind::Png,
        ],
    }
}

pub fn parse_output_kind(value: &str) -> Result<OutputKind, GithubStatsError> {
    match value.trim().to_ascii_lowercase().as_str() {
        "dashboard" => Ok(OutputKind::Dashboard),
        "stats" => Ok(OutputKind::Stats),
        "languages" | "top-languages" | "top-langs" => Ok(OutputKind::Languages),
        "streak" => Ok(OutputKind::Streak),
        "heat" | "heat-ring" | "ring" => Ok(OutputKind::Heat),
        "metric" | "figure" => Ok(OutputKind::Metric),
        "repo" | "repository" => Ok(OutputKind::Repo),
        "gist" => Ok(OutputKind::Gist),
        "activity" | "coding-activity" => Ok(OutputKind::Activity),
        "activity-readme" | "coding-activity-readme" => Ok(OutputKind::ActivityReadme),
        "status" => Ok(OutputKind::Status),
        "json" => Ok(OutputKind::Json),
        "png" => Ok(OutputKind::Png),
        _ => Err(GithubStatsError::UnsupportedOutputKind {
            value: value.to_owned(),
        }),
    }
}
