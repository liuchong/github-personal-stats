...
fn workspace_info_exposes_dashboard_as_default() {
    ...
    assert!(info.supported_outputs.contains(&OutputKind::ActivityReadme));
}
...
fn workspace_info_serializes_to_stable_json() {
    ...
    assert!(json.contains(r#""activity-readme""#));
}
...
fn output_kind_parser_accepts_canonical_values() {
    ...
    assert_eq!(parse_output_kind("activity"), Ok(OutputKind::Activity));
    assert_eq!(
        parse_output_kind("activity-readme"),
        Ok(OutputKind::ActivityReadme)
    );
